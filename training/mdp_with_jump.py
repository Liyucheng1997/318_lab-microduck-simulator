...
# ─────────────────────────────────────────────────────────────────────────────
# Jump task — vertical hop from a standing start, land, return to stand.
#
# State machinery mirrors the roulade accumulator pattern (step-guarded
# frontier accounting + a hard state latch), with FLIGHT taking the role
# rotation played there:
#   • flight     = no robot-terrain contact AND trunk upright — an upright
#     ballistic phase. The upright factor stops "tip over" (feet leave the
#     ground on the way down) from reading as flight.
#   • latch      = ≥ JUMP_LATCH_STEPS consecutive flight steps. Guards against
#     the 1-step spawn-settle "flight" (reset drops the robot ~1 mm) opening
#     the landing gate at birth.
#   • apex       = frontier (max-so-far) of trunk height reached DURING
#     latched flight. Only the frontier pays → the best jump pays once;
#     bouncing repeatedly re-earns nothing (roulade paid-pointer lesson).
#   • completion = smoothstep(apex − stand_height, gate_lo, gate_hi) × latch —
#     the landing annuity opens in proportion to how high the robot actually
#     flew, so a 2 mm tiptoe hop unlocks ≈nothing and the gradient points UP.
# ─────────────────────────────────────────────────────────────────────────────

_JUMP_SUPPORT_SENSOR = "robot_ground_contact"  # any robot geom ↔ terrain
_JUMP_UPRIGHT_MIN = 0.7    # body z-axis world-z (cos tilt) to count as upright
_JUMP_LATCH_STEPS = 2      # consecutive flight steps required to latch


def _jump_body_upright(asset: Entity) -> torch.Tensor:
    """World-z component of the trunk z-axis (1 = vertical, −1 = inverted)."""
    quat = asset.data.root_link_quat_w
    return 1.0 - 2.0 * (quat[:, 1].pow(2) + quat[:, 2].pow(2))


def _jump_state(env: "ManagerBasedRlEnv") -> None:
    if not hasattr(env, "_jump_apex_z"):
        z = torch.zeros(env.num_envs, device=env.device)
        env._jump_apex_z = z.clone()        # frontier of trunk z during flight
        env._jump_paid_apex = z.clone()     # paid pointer on the apex frontier
        env._jump_air_steps = z.clone()     # cumulative latched-flight steps
        env._jump_air_paid = z.clone()      # paid pointer on air steps
        env._jump_takeoff_paid = z.clone()  # paid vz·dt integral (bootstrap cap)
        env._jump_consec_flight = z.clone() # consecutive flight-step counter
        env._jump_latch = torch.zeros(env.num_envs, dtype=torch.bool, device=env.device)
        env._jump_last_update_step = -1


def _update_jump_state(env: "ManagerBasedRlEnv", asset: Entity) -> None:
    """Advance the per-env jump accounting once per control step (step-guarded)."""
    _jump_state(env)
    step = int(env.common_step_counter)
    if step == env._jump_last_update_step:
        return
    supported = _sensor_any_contact(env, _JUMP_SUPPORT_SENSOR)
    if supported is None:  # sensor missing (shouldn't happen) → all supported
        supported = torch.ones(env.num_envs, dtype=torch.bool, device=env.device)
    upright = _jump_body_upright(asset) > _JUMP_UPRIGHT_MIN
    flight = (~supported) & upright

    env._jump_consec_flight = torch.where(
        flight, env._jump_consec_flight + 1.0, torch.zeros_like(env._jump_consec_flight)
    )
    latched_now = env._jump_consec_flight >= _JUMP_LATCH_STEPS
    env._jump_latch = env._jump_latch | latched_now

    z = torch.nan_to_num(
        asset.data.root_link_pos_w[:, 2] - env.scene.terrain.env_origins[:, 2], nan=0.0
    )
    in_latched_flight = flight & latched_now
    env._jump_apex_z = torch.maximum(
        env._jump_apex_z, torch.where(in_latched_flight, z, torch.zeros_like(z))
    )
    env._jump_air_steps = env._jump_air_steps + in_latched_flight.float()
    env._jump_last_update_step = step


def _jump_completion_gate(
    env: "ManagerBasedRlEnv",
    stand_height: float,
    gate_lo: float,
    gate_hi: float,
) -> torch.Tensor:
    """Smoothstep on the flight apex rise above standing height × flight latch."""
    _jump_state(env)
    rise = env._jump_apex_z - stand_height
    t = torch.clamp((rise - gate_lo) / max(gate_hi - gate_lo, 1e-6), 0.0, 1.0)
    return (t * t * (3.0 - 2.0 * t)) * env._jump_latch.float()
...
def reset_jump_state(
    ...
    """Zero the jump accounting on episode reset (spawn pose is the base
    velocity-env standing reset; no custom spawn needed for this task)."""
    _jump_state(env)
    if env_ids is None or len(env_ids) == 0:
        return
    env_ids = env_ids.to(env.device, dtype=torch.long)
    env._jump_apex_z[env_ids] = 0.0
    env._jump_paid_apex[env_ids] = 0.0
    env._jump_air_steps[env_ids] = 0.0
    env._jump_air_paid[env_ids] = 0.0
    env._jump_takeoff_paid[env_ids] = 0.0
    env._jump_consec_flight[env_ids] = 0.0
    env._jump_latch[env_ids] = False
...
def jump_height_progress(
    ...
    """Pay increments of the flight-apex frontier, up to target_rise above stand.

    reward = Δ(clamp(apex − stand_height, 0, target_rise)) / (step_dt · target)
    — the roulade_progress recipe applied to height: potential-based (camping
    pays zero), frontier-based (the best jump pays once; re-hopping to the
    same apex pays nothing), and only height gained during LATCHED UPRIGHT
    FLIGHT counts (standing tall, tiptoes, or a tilted fling earn nothing).
    Total payout for a full target_rise jump ≈ weight / step_dt = weight × 50.
    """
    asset: Entity = env.scene[asset_cfg.name]
    _update_jump_state(env, asset)
    new_paid = torch.clamp(env._jump_apex_z - stand_height, min=0.0, max=target_rise)
    delta = torch.clamp(new_paid - env._jump_paid_apex, min=0.0)
    env._jump_paid_apex = torch.maximum(env._jump_paid_apex, new_paid)
    return delta / (env.step_dt * target_rise)


def jump_airtime(
    env: "ManagerBasedRlEnv",
    max_paid_steps: float = 30.0,
    asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
) -> torch.Tensor:
    """1.0 per latched-flight step, capped at max_paid_steps per episode.

    The cap (≈0.6 s of paid air) removes the incentive to farm airtime with
    endless micro-hops once a real jump exists; the height frontier is the
    signal that scales with jump quality, this is just the discovery carrot.
    """
    asset: Entity = env.scene[asset_cfg.name]
    _update_jump_state(env, asset)
    new_paid = torch.clamp(env._jump_air_steps, max=max_paid_steps)
    delta = torch.clamp(new_paid - env._jump_air_paid, min=0.0)
    env._jump_air_paid = torch.maximum(env._jump_air_paid, new_paid)
    return delta


def jump_takeoff_velocity(
    env: "ManagerBasedRlEnv",
    max_paid_meters: float = 0.10,
    asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
) -> torch.Tensor:
    """Upward trunk velocity while supported and upright, capped by integral.

    The discovery bootstrap: before any flight exists the gated landing terms
    are all zero and the height frontier needs a first liftoff to pay — this
    term makes "push the ground away fast" profitable from step 0. Payment is
    capped at max_paid_meters of integrated upward travel per episode, so
    bobbing up and down farms it once and then it goes silent (no jackpot).
    """
    asset: Entity = env.scene[asset_cfg.name]
    _update_jump_state(env, asset)
    supported = _sensor_any_contact(env, _JUMP_SUPPORT_SENSOR)
    if supported is None:
        supported = torch.ones(env.num_envs, dtype=torch.bool, device=env.device)
    upright = _jump_body_upright(asset) > _JUMP_UPRIGHT_MIN
    vz = torch.clamp(torch.nan_to_num(asset.data.root_link_lin_vel_w[:, 2], nan=0.0), min=0.0)
    pay_m = torch.minimum(
        vz * env.step_dt, torch.clamp(max_paid_meters - env._jump_takeoff_paid, min=0.0)
    ) * (supported & upright).float()
    env._jump_takeoff_paid = env._jump_takeoff_paid + pay_m
    return pay_m / env.step_dt
...
def jump_landing_composite(
    ...
    """standing_composite_score × jump completion gate — the landing annuity.

    Once the robot has actually flown (latch) high enough (apex smoothstep),
    every step spent standing at HOME pose pays — sticking the landing and
    staying up dominates every crash outcome. Exactly zero for the standing
    spawn that never jumps, so it cannot be farmed by doing nothing.
    """
    asset: Entity = env.scene[asset_cfg.name]
    _update_jump_state(env, asset)
    score = standing_composite_score(
    ...
    return score * _jump_completion_gate(env, target_height, gate_lo, gate_hi)


def jump_upright_after(
    env: "ManagerBasedRlEnv",
    target_height: float,
    gate_lo: float = 0.008,
    gate_hi: float = 0.025,
    ...
    """Linear cos(tilt) × completion gate — broad pull toward vertical after
    the jump (gradient where the composite product is ≈0, the standup
    two-layer lesson)."""
    asset: Entity = env.scene[asset_cfg.name]
    _update_jump_state(env, asset)
    upright = torch.clamp(_jump_body_upright(asset), min=0.0)
    return upright * _jump_completion_gate(env, target_height, gate_lo, gate_hi)
...
def jump_stand_tax(
    ...
    """SELF-NEGATING height L1 below target × completion gate → POSITIVE weight.

    The roulade run-3 lesson: gated positive landing rewards make standing
    better than a heap, but the heap itself is free — "jump then crumple"
    parks in a ≈0/step basin. This makes every post-jump step spent below
    standing height net-negative, forcing the rise. The gate keeps the
    crouch-and-launch itself untaxed (it happens pre-latch)."""
    asset: Entity = env.scene[asset_cfg.name]
    _update_jump_state(env, asset)
    z = torch.nan_to_num(
        asset.data.root_link_pos_w[:, 2] - env.scene.terrain.env_origins[:, 2], nan=0.0
    ...
    shortfall = torch.clamp(target_height - z, min=0.0)
    return -shortfall * _jump_completion_gate(env, target_height, gate_lo, gate_hi)
